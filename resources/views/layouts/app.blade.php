...
<head>
    ...
    <style>
        ...
        .glass-card:hover {
            ...
            box-shadow: 0 14px 40px -10px rgba(99, 102, 241, 0.25), 0 0 25px -5px rgba(168, 85, 247, 0.15);
        ...
        .border-glow::before {
            ...
            padding: 1px;
            background: linear-gradient(135deg, rgba(99, 102, 241, 0.5), rgba(168, 85, 247, 0.3), rgba(59, 130, 246, 0.4));
        ...
        .dark-ambient-bg {
            ...
            background-image:
                ...
                radial-gradient(at 50% 100%, rgba(16, 185, 129, 0.1) 0px, transparent 55%),
                radial-gradient(at 80% 50%, rgba(59, 130, 246, 0.12) 0px, transparent 50%);
        }
    </style>
</head>
<body class="h-full text-slate-100 dark-ambient-bg flex flex-col antialiased selection:bg-indigo-500 selection:text-white"
    ...
    <header class="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-2xl border-b border-slate-800/80 shadow-2xl" x-data="{ mobileMenuOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ Auth::user()->role === 'admin' ? route('admin.dashboard') : route('tutor.dashboard') }}" class="group flex items-center space-x-3 transition-all duration-200 active:scale-95">
                        <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-500 via-indigo-600 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/30 group-hover:shadow-indigo-500/50 group-hover:scale-105 transition-all duration-300 border border-indigo-400/30">
                        ...
                        <div>
                            <span class="text-white font-heading font-extrabold text-lg tracking-tight flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 shadow-xs">v2.5</span>
                            </span>
                        </div>
                    </a>
                </div>
                ...
                <div class="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <button @click="cmdPaletteOpen = true" class="hidden md:flex items-center space-x-3 px-3.5 py-1.5 bg-slate-900/90 hover:bg-slate-800/90 border border-slate-700/80 text-slate-400 hover:text-slate-200 text-xs font-medium rounded-2xl transition-all shadow-inner group">
                        <i class="fas fa-search text-indigo-400 group-hover:scale-110 transition-transform"></i>
                    ...
                    <div class="relative">
                        <button id="notificationBtn" class="relative p-2.5 text-slate-400 hover:text-indigo-400 hover:bg-slate-800/80 rounded-2xl transition-all duration-200 focus:outline-none border border-transparent hover:border-slate-700/80">
                            <i class="fas fa-bell text-lg"></i>
                            <span class="absolute top-2 right-2 w-2.5 h-2.5 bg-indigo-500 rounded-full ring-2 ring-slate-950 animate-pulse"></span>
                        </button>
                    </div>
                    ...
                    <div class="relative group" x-data="{ open: false }">
                        <button @click="open = !open" @click.away="open = false" class="flex items-center space-x-3 p-1.5 rounded-2xl hover:bg-slate-800/80 border border-transparent hover:border-slate-700/80 transition-all duration-200 focus:outline-none">
                        ...
                        <div x-show="open"
                             ...
                             class="absolute right-0 mt-2 w-56 bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-slate-800 py-2 z-50 divide-y divide-slate-800"
                             style="display: none;">
                            <div class="px-4 py-2.5">
                                <p class="text-xs text-slate-400 font-medium">Signed in as</p>
                                <p class="text-sm font-bold text-slate-200 truncate font-heading">{{ Auth::user()->email ?? Auth::user()->name }}</p>
                            </div>
                            <div class="py-1">
                    ...
                    <!-- Mobile Menu Button -->
                    <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden p-2 text-slate-400 hover:bg-slate-800 rounded-2xl transition-all">
        ...
        <div x-show="mobileMenuOpen" x-transition class="lg:hidden border-t border-slate-800 bg-slate-900/95 backdrop-blur-2xl px-4 py-5 shadow-2xl space-y-4">
            <nav class="space-y-1">
                @if(Auth::user()->role === 'admin')
                <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-300 hover:bg-slate-800' }}">
                    <i class="fas fa-chart-pie w-5 text-indigo-400"></i>
                    <span>Dashboard</span>
                </a>
                @else
                <a href="{{ route('tutor.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold {{ request()->routeIs('tutor.dashboard') ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-300 hover:bg-slate-800' }}">
                    <i class="fas fa-chart-pie w-5 text-indigo-400"></i>
                    <span>Tutor Dashboard</span>
                </a>
                @endif
                <a href="{{ route('quizzes.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold {{ request()->routeIs('quizzes.*') ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-300 hover:bg-slate-800' }}">
                    <i class="fas fa-circle-question w-5 text-purple-400"></i>
                    <span>Manage Quizzes</span>
                </a>
                @if(Auth::user()->role === 'admin')
                <a href="{{ route('students.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold {{ request()->routeIs('students.*') || request()->routeIs('admin.students') ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-300 hover:bg-slate-800' }}">
                    <i class="fas fa-user-graduate w-5 text-blue-400"></i>
                    <span>Students</span>
                </a>
                @endif
                <a href="{{ route('courses.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-2xl text-sm font-semibold {{ request()->routeIs('courses.*') ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-300 hover:bg-slate-800' }}">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full flex flex-col lg:flex-row gap-8">
        ...
        <aside class="w-64 hidden lg:block shrink-0">
            <div class="sticky top-24 glass-panel rounded-3xl p-4 space-y-6 shadow-2xl border-glow">

                <!-- Main Nav Group -->
                <div>
                    <p class="px-3.5 text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2 font-heading">Core Navigation</p>
                    <nav class="space-y-1.5">
                        @if(Auth::user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}" class="group flex items-center justify-between px-3.5 py-2.5 rounded-2xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-gradient-to-r from-indigo-600/30 to-purple-600/20 text-indigo-300 font-bold border border-indigo-500/35 shadow-md shadow-indigo-950/50' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200 hover:translate-x-1' }}">
                ...
                <div class="pt-4 border-t border-slate-800/80">
                    <p class="px-3.5 text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2 font-heading">Control Tools</p>
                    <nav class="space-y-1">
                        <button @click="cmdPaletteOpen = true" class="w-full text-left group flex items-center space-x-3 px-3.5 py-2.5 rounded-2xl text-sm font-medium text-slate-400 hover:bg-slate-800/60 hover:text-slate-200 transition hover:translate-x-1">
                ...
                <div class="p-4 bg-gradient-to-br from-slate-900 via-indigo-950/80 to-purple-950/50 rounded-2xl text-white border border-slate-800 shadow-xl">
    ...
    <div x-show="cmdPaletteOpen"
         ...
         class="fixed inset-0 z-50 overflow-y-auto p-4 sm:p-6 md:p-20 bg-slate-950/85 backdrop-blur-md flex items-start justify-center"
         style="display: none;"
         @click.away="cmdPaletteOpen = false">

        <div class="relative w-full max-w-2xl bg-slate-900/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-slate-800 overflow-hidden divide-y divide-slate-800">
            ...
            <div class="p-4 space-y-2 max-h-96 overflow-y-auto">
                <p class="px-3 text-[11px] font-bold uppercase tracking-wider text-slate-400 font-heading">Quick Actions</p>
    ...
    <footer class="mt-auto border-t border-slate-800/80 bg-slate-950/60 backdrop-blur-md py-6 text-slate-400 text-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-2.5">
                <div class="w-6 h-6 rounded-lg bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-md">A</div>
                <p class="text-xs font-medium text-slate-400">&copy; {{ date('Y') }} LMS Control Center. Crafted with high precision.</p>
            </div>
                ...
                html: `
                    <div class="text-left space-y-3 mt-3">
                        <div class="p-3 bg-indigo-500/10 rounded-2xl border border-indigo-500/20 flex items-start space-x-3">
                            ...
                            <div>
                                <p class="font-bold text-slate-200 text-xs font-heading">New Student Registrations</p>
                        ...
                        <div class="p-3 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 flex items-start space-x-3">
                            ...
                            <div>
                                <p class="font-semibold text-slate-200 text-xs font-heading">Assessment Submissions</p>
